defmodule MustchanWeb.UserAuthTest do
  use MustchanWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Mustchan.Accounts
  alias Mustchan.Accounts.Scope
  alias MustchanWeb.UserAuth

  import Mustchan.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MustchanWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      MustchanWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        MustchanWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
